...
class SAM2ImageDecoder(nn.Module):
    ...
    def forward(
        ...
        masks = F.interpolate(
            masks, (img_size[0], img_size[1]), mode="bilinear", align_corners=False
        )
...
def export_encoder(model_type, sam2_model):
...
def export_decoder(sam2_model, high_res_feats_0, high_res_feats_1, image_embed):
...
if __name__ == "__main__":
    ...
    sam2_checkpoint = f"lpcvc_track2_models/{model_type}.pt"
    sam2_model = build_sam2(model_cfg, sam2_checkpoint, device="cpu")
    high_res_feats_0, high_res_feats_1, image_embed = export_encoder(
        model_type, sam2_model
    )
    export_decoder(sam2_model, high_res_feats_0, high_res_feats_1, image_embed)
