...
def all_gather_pickle(data, device):
    ...
    local_size = torch.LongTensor([tensor.numel()]).cpu()
    size_list = [torch.LongTensor([0]).cpu() for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.ByteTensor(size=(max_size,)).cpu())
    if local_size != max_size:
        padding = torch.ByteTensor(size=(max_size - local_size,)).cpu()
