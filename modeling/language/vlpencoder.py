...
class LanguageEncoder(nn.Module):
    ...
    def get_text_embeddings(self, class_names, name='default', is_eval=False, add_bgd=False, prompt=True, norm=True, store_buffer=None):
        if not is_eval:
            ...
            text_emb = self.forward_language((arbitary_tokens.cpu(), arbitary_attention_masks.cpu()), norm=norm)
            ...
            with torch.no_grad():
                def extract_mean_emb(txts):
                    ...
                    clss_embedding = self.forward_language((tokens['input_ids'].cpu(), tokens['attention_mask'].cpu()), norm=norm)
    ...
    def get_text_token_embeddings(self, txts, name='default', token=False, norm=False):
        if not token:
            ...
            tokens = {key: value.cpu() for key, value in tokens.items()}
        else:
            tokens = txts
