...
from sam2_onnx import SAM2Image
...
class GroundedSAMOnnx(nn.Module):

    def __init__(self, object_detection_model: GroundingDino, encoder_model_path, decoder_model_path, device='cpu'):
        super().__init__()
        self.object_detection_model = object_detection_model
        self.sam_predictor = SAM2Image(encoder_model_path, decoder_model_path)
        self.image_resolution = 1024
    ...
    def forward(self, image_input, text_input, image_save_dir: Path = Path("./compile_and_profile/annotated")):
        ...
        def segment(sam_predictor: SamPredictor, image: np.ndarray, xyxy: np.ndarray) -> np.ndarray:
            ...
            for box in xyxy:
                ...
                sam_predictor.set_box(box, 0)
                masks = sam_predictor.get_masks()
                for label_id, label_masks in masks.items():
                    if label_masks is None:
                        continue
                    result_masks.append(label_masks)
            return np.array(result_masks)
