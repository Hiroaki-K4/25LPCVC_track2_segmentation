...
def build_baseline_model(
    image_input,
    text_input,
    output_path="./compile_and_profile",
    test_torch_model_local=True,
):
    """
    ...
    backbone.load_state_dict(
        {k.replace("backbone.", ""): v for k, v in ckpt.items() if "backbone" in k},
        strict=True,
    )
    ...
    input_shape = {
        "res2": ShapeSpec(
            channels=backbone_out_feats[0], stride=backbone_out_strides[0]
        ),
        "res3": ShapeSpec(
            channels=backbone_out_feats[1], stride=backbone_out_strides[1]
        ),
        "res4": ShapeSpec(
            channels=backbone_out_feats[2], stride=backbone_out_strides[2]
        ),
        "res5": ShapeSpec(
            channels=backbone_out_feats[3], stride=backbone_out_strides[3]
        ),
    }

    multi_scale_feature_extractor = build_encoder(opt, input_shape).to(device)
    multi_scale_feature_extractor.load_state_dict(
        {
            k.replace("sem_seg_head.pixel_decoder.", ""): v
    ...
    lang_encoder.load_state_dict(
        {
            k.replace("sem_seg_head.predictor.lang_encoder.", ""): v
            for k, v in ckpt.items()
            if "sem_seg_head.predictor.lang_encoder" in k
        },
        strict=True,
    )
    ...
    mask_decoder.load_state_dict(
        {
            k.replace("sem_seg_head.predictor.", ""): v
            for k, v in ckpt.items()
            if (
                ("sem_seg_head.predictor" in k)
                and ("sem_seg_head.predictor.lang_encoder" not in k)
            )
        },
        strict=False,
    )
    ...
    model = XDecoder(
        opt, backbone, multi_scale_feature_extractor, lang_encoder, mask_decoder
    )
...
def inference_torch_local(
    model, image_input, text_input, output_path="./compile_and_profile"
):
    """
    ...
    pred_mask = (
        pred_mask.detach().cpu().numpy() if device == "cuda" else pred_mask.numpy()
    )
...
class XDecoder(nn.Module):
    ...
    def forward(self, image_input, text_input):
        ...
        visual_features = self.backbone(
            images
        )  # return dict, {'res2', 'res3', 'res4', 'res5'}
        mask_features, _, multi_scale_features = self.feature_extractor(visual_features)

        text_embeddings = self.lang_encoder.get_text_token_embeddings(
            tokens, name="grounding", token=True, norm=False
        )
        ...
        top1_mask_pred = self.post_processing(
            output["pred_masks"],
            output["pred_captions"],
            text_embeddings["class_emb"][0],
        )
